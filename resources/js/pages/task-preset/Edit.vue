...
const props = defineProps<{
  preset: { data: PresetEditPayload } | PresetEditPayload
  departments: { data: Department[] }
}>()

const preset = computed(() => ('data' in props.preset ? props.preset.data : props.preset))
...
const form = reactive({
  ...
  department_id: preset.value.department_id ?? OPEN_TO_ANYONE,
})

const steps = ref<TaskStep[]>(
  (preset.value.steps ?? []).map((step, idx) => ({
    id: step.id ?? uid(),
...
const submit = () => {
  ...
  router.patch(taskPreset.update(preset.value.id).url, payload)
}
</script>

<template>
  <Head title="Edit Task Preset" />

  <AppLayout>
    <div class="flex flex-col gap-4 p-4">
      <div class="flex items-start justify-between gap-3">
        <div>
          <h1 class="text-2xl font-semibold tracking-tight">Edit Task Preset</h1>
        </div>
